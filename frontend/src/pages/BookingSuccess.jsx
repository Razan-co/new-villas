...
export default function BookingSuccess() {
  ...
  const { getMyBookings, bookings, loading } = useBookingStore();

  // Fetch booking data using ID or location state
  useEffect(() => {
    if (id) {
      getMyBookings();
    }
  ...
  // Find the current booking by ID, fallback to location state
  const booking = bookings.find(b => b._id === id) || location.state?.booking || {
    checkIn: new Date(),
    checkOut: new Date(),
    days: 3,
    price: 48500,
  ...
  if (loading && !booking.fullName) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950">
        <div className="text-slate-400">Loading your booking details...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-2xl rounded-3xl bg-slate-900/70 border border-slate-800/80 shadow-[0_24px_80px_rgba(0,0,0,0.75)] backdrop-blur-xl p-8 md:p-10 text-slate-50">
        {/* Icon + heading */}
        <div className="flex flex-col items-center text-center gap-4 mb-8">
          <div className="h-16 w-16 rounded-full bg-emerald-500/10 flex items-center justify-center border border-emerald-500/40">
          ...
          <div>
            ...
            <p className="mt-2 text-sm md:text-base text-slate-400 max-w-lg mx-auto">
              Thank you for choosing our villa retreat. Your stay is now reserved and our team will contact you shortly.
            </p>
          </div>
        </div>

        {/* Booking details card - using YOUR exact backend fields */}
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 px-6 py-5 md:px-7 md:py-6 mb-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="space-y-2">
              ...
              <p className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500 mt-3">
                Guests
              </p>
              <p className="text-base md:text-lg text-slate-100">
            ...
            <div className="border-t border-slate-800 pt-4 md:border-l md:border-t-0 md:pl-6 md:pt-0 md:text-right">
              <p className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500">
                Total amount
              </p>
              ...
              <p className="mt-2 text-xs text-slate-500">
                {booking.days} nights • Taxes included
              </p>
            </div>
          </div>
        </div>
        ...
        <p className="text-sm text-slate-400 text-center mb-8">
          Our reservation team will contact you on {booking.phone} to finalize arrangements and assist with any special requests.
        </p>
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-3">
          <Link
            to="/"
            className="inline-flex w-full md:w-auto items-center justify-center rounded-full bg-emerald-500 px-6 py-2.5 text-sm font-medium text-slate-950 shadow-lg shadow-emerald-500/25 hover:bg-emerald-400 transition-colors"
          >
            Back to home
          </Link>
          <Link
            to="/my-bookings"
            className="inline-flex w-full md:w-auto items-center justify-center rounded-full border border-slate-700 px-6 py-2.5 text-sm font-medium text-slate-100 hover:border-slate-500 hover:bg-slate-900/60 transition-colors"
          >
            View my bookings
          </Link>
        </div>
        ...
        {id && (
          <p className="mt-6 text-center text-xs text-slate-500">
            Booking reference: <span className="font-medium text-slate-300">#{id}</span>
          </p>
        )}
      </div>
    </div>
